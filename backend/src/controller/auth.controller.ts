...
export class AuthController {
  ...
  register = async (
    ...
    try {
      ...
      res
        .status(201)
        .json({ user: user, message: "user registered Successfylly" });
  ...
  login = async (
    ...
    try {
      ...
      res.status(200).json({
        ...
        message: "user loggedin Successfylly",
      });
  ...
  getRefreshToken = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res
        .status(201)
        .json({ access_token: at, message: "Token generation successfull" });
    } catch (error) {
      next(error);
    }
  };
  ...
  handleGoogleOauthSuccess = async (
    ...
    try {
      const { at, rt } = await this.authService.handleGoogleOauthSuccess(
        req.user
      );
      ...
      res
        .status(201)
        .json({ access_token: at, message: "user logged in successful" });
    } catch (error) {
      next(error);
    }
  };
